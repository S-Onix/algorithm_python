...
#    - 해당 간선에 연결된 인접 정점이 '연결된 노드 집합'에 이미 들어 있다면, 
#      스킵함(cycle 발생을 막기 위함)
#    - 해당 간선에 연결된 인접 정점이 '연결된 노드 집합'에 들어 있지 않으면, 
#      해당 간선을 선택하고, 해당 간선 정보를 '최소 신장 트리(mst)'에 삽입
#      - 해당 간선에 연결된 인접 정점의 간선들 중, '연결된 노드 집합(connected_nodes)' 에 없는 노드와 
#        연결된 간선들만 간선 리스트(candidate_edge_list) 에 삽입 
#        - '연결된 노드 집합(connected_nodes)' 에 있는 노드와 연결된 간선들을 간선 리스트에 삽입해도, 
#          해당 간선은 스킵될 것이기 때문임
#        - 어차피 스킵될 간선을 간선 리스트(candidate_edge_list)에 넣지 않으므로 해서, 
#          간선 리스트(candidate_edge_list)에서 최소 가중치를 가지는 간선부터 추출하기 위한 
